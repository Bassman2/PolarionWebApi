...
[TestClass]
public class PolarionCollectionsUnitTest : PolarionBaseUnitTest
{
    [TestMethod]
    public async Task TestMethodGetCollectionsAsync()
    {
        using var polarion = new Polarion(storeKey, appName);

        var item = await polarion.GetCollectionsAsync(projectId);
